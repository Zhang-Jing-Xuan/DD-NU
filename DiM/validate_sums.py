...
def train_match_model(args, model, optim_model, img_syn_cache,lab_syn_cache, criterion):
    '''The training function for the match model
    '''
    for batch_idx in range(img_syn_cache.shape[0]):
        img = img_syn_cache[batch_idx]
        lab = lab_syn_cache[batch_idx]
...
def distill(args, img_syn_cache, lab_syn_cache, ipc, e_model,cnt):
    if ipc==0:
        ipc=1
    img_syn = img_syn_cache[-ipc*args.num_classes:].cuda().requires_grad_(True)
    lab_syn = lab_syn_cache[-ipc*args.num_classes:].cuda()
    optim_img = torch.optim.SGD([img_syn], lr=0.01, momentum=0.5)
    for it in range(2):
        ...
        model = define_model(args, args.num_classes, e_model).cuda()
        model.load_state_dict(torch.load("/home/dd/DL/DataDistillation/Acc-DD/pretrained_models/cifar10/conv3in_cut_seed_2023_lr_0.01_aug_color_crop_cutout/checkpoint_best_0.pth.tar"))
        # criterion = nn.CrossEntropyLoss()
        loss_total = 0
        for inner in range(1): # 1
            for c in range(args.num_classes):
                img_syn_s,lab_syn_s=img_syn[c*ipc:(c+1)*ipc],lab_syn[c*ipc:(c+1)*ipc]
                img_real_s,lab_real_s=img_syn_cache[c*ipc:(c+1)*ipc],lab_syn_cache[c*ipc:(c+1)*ipc]
                
                for i in range(1,cnt-1):
                    img_real_s=torch.cat([img_real_s,img_syn_cache[ipc*i+c*ipc:ipc*i+(c+1)*ipc]])
                    lab_real_s=torch.cat([lab_real_s,lab_syn_cache[ipc*i+c*ipc:ipc*i+(c+1)*ipc]])

                n = img_real_s.shape[0]
                img_aug = torch.cat([img_real_s, img_syn_s])

                loss = matchloss(args, img_aug[:n], img_aug[n:], lab_real_s, lab_syn_s, model)
                loss_total += loss.item()

                optim_img.zero_grad()
                loss.backward()
                optim_img.step()
            # train_match_model(args,model,optim_model,img_syn_cache,lab_syn_cache,criterion)
    return img_syn, lab_syn

def matchloss_model(args, img_syn, lab_syn, model_curr, model):
    loss = None
    # criterion = nn.CrossEntropyLoss()

    # output_real = model(img_syn)
    # loss_real = criterion(output_real, lab_syn)
    # g_real = torch.autograd.grad(loss_real, model.parameters())
    # g_real = list((g.detach() for g in g_real))

    # output_syn = model_curr(img_syn)
    # loss_syn = criterion(output_syn, lab_syn)
    # g_syn = torch.autograd.grad(loss_syn, model_curr.parameters(), create_graph=True)
    ...
    with torch.no_grad():
        feat_tg = model.get_feature(img_syn, 0, 3)
    feat = model_curr.get_feature(img_syn, 0, 3)

    for i in range(len(feat)):
        loss = add_loss(loss, dist(feat_tg[i].mean(0), feat[i].mean(0), method=args.metric))
    return loss

def distill_model(args, img_syn,lab_syn,model_curr,model):
    optim_img = torch.optim.SGD([img_syn], lr=0.01, momentum=0.5)
    for it in range(2):
        model.eval()
        model_curr.eval()
        ...
        for inner in range(1): # 1
            ...
            loss = matchloss_model(args, img_syn, lab_syn, model_curr, model)
            loss_total += loss.item()
...
def validate(args, generator, testloader, criterion, aug_rand):
    ...
    for e_model in args.eval_model:
        ...
        cnt=0
        img_syn_cache, lab_syn_cache = torch.Tensor([]),torch.Tensor([])
        for epoch_idx in range(args.epochs_eval):
            for batch_idx in range(10 * args.ipc // args.batch_size):
                ...
                if args.batch_size == args.num_classes:
                    ...
                    lab_syn = torch.randint(args.num_classes, (args.batch_size,))
                ## ipc = 10: 2*args.ipc//args.num_classes
                ## ipc = 1:  10*args.ipc//args.num_classes
                lab_syn=torch.stack([torch.ones(2*args.ipc//args.num_classes).long()*i for i in range(0,args.num_classes)]).view(-1).cuda()
                noise = torch.normal(0, 1, (args.batch_size, args.dim_noise))
                lab_onehot = torch.zeros((args.batch_size, args.num_classes))
                lab_onehot[torch.arange(args.batch_size), lab_syn] = 1
                noise[torch.arange(args.batch_size), :args.num_classes] = lab_onehot[torch.arange(args.batch_size)]
                ...
                if cnt==0:
                    cnt=1
                    img_syn_cache, lab_syn_cache=img_syn, lab_syn
                elif cnt<10: # ipc=50 ,cnt<10
                    cnt=cnt+1
                    img_syn_cache=torch.cat([img_syn_cache,img_syn])
                    lab_syn_cache=torch.cat([lab_syn_cache,lab_syn])
                    img_syn,lab_syn=distill(args, img_syn_cache,lab_syn_cache,2*args.ipc//args.num_classes,e_model,cnt)
                else:
                    img_syn_cache=torch.cat([img_syn_cache[img_syn.shape[0]:],img_syn])
                    lab_syn_cache=torch.cat([lab_syn_cache[lab_syn.shape[0]:],lab_syn])
                    img_syn,lab_syn=distill(args, img_syn_cache,lab_syn_cache,2*args.ipc//args.num_classes,e_model,cnt)
                ...
                if (epoch_idx+1)%10==0: # CIFAR10,IPC=50,%50 IPC=10,%10
                    model_curr=copy.deepcopy(model)
                    ema.apply_shadow()
                    img_syn,lab_syn=distill_model(args, img_syn,lab_syn,model_curr,model)
                    torch.save([img_syn.detach().cpu(), lab_syn.cpu()],os.path.join(args.logs_dir, f'data_best.pt'))
                    ema.restore()
