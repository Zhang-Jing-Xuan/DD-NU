# python gen_condense_cifar100.py --tag gan_cifar100 --batch-size 64  --data cifar100 --num-classes 100 --epochs 120 --dim-noise 80
import os
import sys
import time
import random
...
from utils import AverageMeter, accuracy, Normalize, Logger, rand_bbox, get_time
...
def load_data(args):
    ...
    if args.data == 'cifar100':
        ...
        trainset = datasets.CIFAR100(root=args.data_dir, train=True, download=True,
                                    transform=transform_train)
        testset = datasets.CIFAR100(root=args.data_dir, train=False, download=True,
                                   transform=transform_test)
        # # 选择您感兴趣的部分类别
        # selected_classes = [i for i in range(0,10)]  # 替换为您所需的类别索引

        # # 创建一个子集，仅包含所选类别的样本
        # subset_indices_train = [idx for idx, (_, target) in enumerate(trainset) if target in selected_classes]
        # subset_indices_test = [idx for idx, (_, target) in enumerate(testset) if target in selected_classes]
        # # 创建Subset对象，包含所选类别的子集
...
def train(args, epoch, generator, discriminator, optim_g, optim_d, trainloader, criterion, aug, aug_rand):
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    Premodel = define_model(args, args.num_classes,"convnet").cuda()
    Premodel.eval()
    checkpoint = torch.load(
            "/home/dd/DL/DataDistillation/Acc-DD/pretrained_models/cifar100/conv3in_2_cut_seed_2023_lr_0.01_aug_color_crop_cutout/checkpoint_best_0.pth.tar"
        )
    ...
    for batch_idx, (img_real, lab_real) in enumerate(trainloader):
        ...
        noise=torch.randn(args.batch_size, args.dim_noise, device="cuda")
        noise = noise.cuda()

        img_syn = generator(noise,lab_real)
        fake_dict = discriminator(img_syn, lab_real)
        gen_loss=g_hinge(fake_dict["adv_output"])
        # gen_class = Premodel(img_syn)
        # gen_loss = criterion(gen_class, lab_real)
        gen_loss.backward()
        optim_g.step()
        ...
        real_dict = discriminator(img_real, lab_real)
        fake_dict = discriminator(img_syn, lab_real)
        

        disc_loss = d_hinge(real_dict["adv_output"], fake_dict["adv_output"])
        disc_loss.backward()
        optim_d.step()
        ...
        #         get_time(),epoch, batch_idx + 1, gen_losses.val, gen_losses.avg, disc_losses.val, disc_losses.avg, top1.val, top1.avg)
        #     )
        if (batch_idx + 1) % args.print_freq == 0:
            print('{} [Train Epoch {} Iter {}] G Loss: {:.3f}({:.3f}) D Loss: {:.3f}({:.3f})'.format(
                get_time(),epoch, batch_idx + 1, gen_losses.val, gen_losses.avg,disc_losses.val, disc_losses.avg,)
            )
...
def validate(args, generator, testloader, criterion, aug_rand):
    ...
    for e_model in args.eval_model:
        ...
        for epoch_idx in range(args.epochs_eval):
            for batch_idx in range(100 * args.ipc // 50):
                # obtain pseudo samples with the generator
                lab_syn = torch.randint(args.num_classes, (args.ipc,))
                noise=torch.randn(args.ipc, args.dim_noise, device="cuda")
...
if __name__ == '__main__':
    ...
    # print(discriminator)
    gpath="/home/dd/DL/DataDistillation/PyTorch-StudioGAN/data/BigGAN/checkpoints/CIFAR100-BigGAN-train-2023_09_20_09_55_42/model=G-best-weights-step=100000.pth"
    generator.load_state_dict(torch.load(gpath)["state_dict"])
    dpath="/home/dd/DL/DataDistillation/PyTorch-StudioGAN/data/BigGAN/checkpoints/CIFAR100-BigGAN-train-2023_09_20_09_55_42/model=D-best-weights-step=100000.pth"
    discriminator.load_state_dict(torch.load(dpath)["state_dict"])
    
    optim_g = torch.optim.Adam(generator.parameters(), lr=0.0002, betas=(0.5, 0.9))
    optim_g.load_state_dict(torch.load(gpath)["optimizer"])
    for state in optim_g.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.cuda()
    optim_d = torch.optim.Adam(discriminator.parameters(), lr=0.0002, betas=(0.5, 0.9))
    optim_d.load_state_dict(torch.load(dpath)["optimizer"])
    ...
    for epoch in range(args.epochs):
        ...
        test_noise=torch.randn(args.batch_size, args.dim_noise, device="cuda")
        # test_noise = torch.normal(-4, 4, (64, args.dim_noise))
        ...
        label=torch.randint(args.num_classes, (args.batch_size,)).cuda()
        test_img_syn = (generator(test_noise,label) + 1.0) / 2.0
        # test_img_syn = generator(test_noise,label)
        test_img_syn = make_grid(test_img_syn.clamp(0.0, 1.0), nrow=8)
        save_image(test_img_syn, os.path.join(args.output_dir, 'outputs/img_{}.png'.format(epoch)))
        generator.train()

        if (epoch + 1) % args.eval_interval == 1:
