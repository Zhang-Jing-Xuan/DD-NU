# python pool_match.py --tag match_kmeans --match logit --match-aug --weight "/home/dd/DL/DataDistillation/DiM/results/gan_baseline/model_dict_119.pth"
import os
import sys
import time
import random
...
def get_init_images(args):
    ...
    for m in model_pool:
        ...
        torch.save(f_c,os.path.join(args.logs_dir, 'f_{}.pt'.format(m)))
...
def train(args, epoch, generator, discriminator, optim_g, optim_d, trainloader, trainset, criterion, aug, aug_rand):
    ...
    top1 = AverageMeter()
    top5 = AverageMeter()
    model_name, model = define_model(args, args.num_classes)
    model = model.cuda()
    model.train()
    optim_model = torch.optim.SGD(model.parameters(), args.eval_lr, momentum=args.momentum,
                                  weight_decay=args.weight_decay)
    f_c = torch.load(os.path.join(args.logs_dir, 'f_{}.pt'.format(model_name)))
    for batch_idx, (img_real, lab_real) in enumerate(trainloader):
        ...
        distance = torch.Tensor([1.]).cuda()
        r_idx = [random.randint(args.fipc*lab_real[idx],args.fipc*(lab_real[idx]+1)-1) for idx in range(lab_real.shape[0])]
        for i in range(0,3):
            f_syn = model.get_feature(img_syn,0,3)[i]
        ...
        gen_loss = gen_loss + 2*distance.cpu().detach().item()*match_loss
        gen_loss.backward()
        optim_g.step()
...
def validate(args, generator, testloader, criterion, aug_rand):
    ...
    all_best_top1 = []
    all_best_top5 = []
    img_init = torch.load(os.path.join(args.logs_dir, f'init_data.pt'))
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        generator.train()
        discriminator.eval()
        train(args, epoch, generator, discriminator, optim_g, optim_d, trainloader, trainset, criterion, aug, aug_rand)
