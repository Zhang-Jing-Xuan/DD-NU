# python pool_match_cifar100.py --tag gan_cifar100 --data cifar100 --num-classes 100 --dim-noise 80 --match logit --match-aug --weight "/home/dd/DL/DataDistillation/PyTorch-StudioGAN/data/BigGAN/checkpoints/CIFAR100-BigGAN-train-2023_09_20_09_55_42/model=G-best-weights-step=100000.pth"
import os
import sys
import time
import random
    ...
    for batch_idx, (img_real, lab_real) in enumerate(trainloader):
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            print('{} [Train Epoch {} Iter {}] G Loss: {:.3f}({:.3f}) '.format(
                get_time(), epoch, batch_idx + 1, gen_losses.val, gen_losses.avg)
            )
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_interval == 1:
            ...
            for _ in range(1):
                top1s, top5s = validate(args, generator, testloader, criterion, aug_rand)
                top1L.append(top1s[0])
            print(f'Repeat {_} => Best acc, std: {np.mean(top1L):.1f} {np.std(top1L):.1f}\n')
            
            for e_idx, e_model in enumerate(args.eval_model):
                top1s[e_idx]=np.mean(top1L)
                ...
                torch.save(
                    model_dict,
                    os.path.join(args.output_dir, 'model_last_dict_{}.pth'.format(e_model)))
                print('Save last model for {}'.format(e_model))
