...
"""
# python sample_latent.py --model DiT-XL/2 --image-size 256 --ckpt ../logs/run-0/000-DiT-XL-2-minimax/checkpoints/0012000.pt --save-dir ../results/dit-distillation/imagenet-10-750 --spec woof --num-samples 750
# python sample_latent.py --model DiT-XL/2 --image-size 256 --ckpt ../logs/run-0/000-DiT-XL-2-minimax/checkpoints/0012000.pt --save-dir ../results/dit-distillation/imagenet-100-woof-play --spec woof --num-samples 10
import os
import torch
...
def main(args):
    ...
    for class_label, sel_class in zip(class_labels, sel_classes):
        ...
        for shift in tqdm(range(args.num_samples // batch_size)):
            ...
            data.append(samples.detach().cpu())
            target.append(class_label)
           
    torch.save([data,target],os.path.join(args.save_dir,f'data.pt'))
