...
"""
# python latent2images.py --model DiT-XL/2 --image-size 256 --ckpt ../logs/run-0/000-DiT-XL-2-minimax/checkpoints/0012000.pt --save-dir ../results/dit-distillation/imagenet-10-100-minimax-distill --spec woof --num-samples 10
import os
import torch
...
def main(args):
    ...
    for class_label, sel_class in zip(class_labels, sel_classes):
        ...
        for shift in tqdm(range(args.num_samples // batch_size)):
            ...
            samples, target = torch.load(args.synpath)

            temp=samples[i].cuda().unsqueeze(0)
            samples = vae.decode(temp / 0.18215).sample
            i=i+1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--synpath", type=str, default=0, help='the path to the synthetic data')
