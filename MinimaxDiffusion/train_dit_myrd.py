...
def mean_of_smallest_k(sim, k, dim=1):
    """
    sim: Tensor, e.g. [a, b]
    k: int, number of smallest values to take
    dim: dimension to operate on
    """
    k_eff = min(k, sim.size(dim))  # 防止不够 k 个
    vals, _ = torch.topk(sim, k=k_eff, dim=dim, largest=False)
    return vals.mean
...
def main(args):
    ...
    # Setup DDP:
    dist.init_process_group("nccl") ### if not in debug, add this line
            ...
            if args.condense:
                ...
                for c in ry_set:
                    if len(pseudo_memory[c]):
                        ...
                        # S=(neg_embeddings-pos_embeddings.mean(dim=0))**2
                        # SS=S.sum(dim=1)*1e-3
                        # My Representativeness constraint
                        sim = (cosine_similarity(
                            pseudo_embeddings[ry == c].flatten(start_dim=1), pos_embeddings
                        ))

                        flat = sim.reshape(-1)           # 展平为 [a*b]
                        k_eff = min(3, flat.numel())
                        vals = flat.topk(
                            k=k_eff,
                            largest=False
                        )
                        pos_feat_sim = 1-vals.values.mean()
                ...
                # neg_match_loss = 0*neg_match_loss ###
