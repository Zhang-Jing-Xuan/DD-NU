    ...
    if plot:
        ...
        if markers is not None and genes is not None:
            ...
            for num, g in enumerate(markers):
                ...
                plt.scatter(meanExpr[i], zeroRate[i], s=10, alpha=0.5,color="k")
...
def plot(
    ...
    **kwargs
    ...
    idmap={193:0,182:1,258:2,162:3,155:4,167:5,159:6,273:7,207:8,229:9}
    y = torch.tensor([idmap[int(x.item())] for x in y])

    x= torch.Tensor([x[i:i+name,:].tolist() for i in range(0,1000,100)]).view(-1,2).numpy()
    y=torch.stack([y[i:i+name] for i in range(0,1000,100)]).view(-1)
    ...
    point_colors = [colors[y[i].item()] for i in range(y.shape[0])]
    # ax.scatter(x[:, 0], x[:, 1], c=point_colors, rasterized=True, **plot_params)
    # plt.scatter(x[:-400, 0], x[:-400, 1], s=0.2,alpha=0.2,c=point_colors[:-400])
    plt.scatter(x[:, 0], x[:, 1], marker="*",s=35,alpha=1,c=point_colors[:])
    # Plot mediods
    if draw_centers:
        centers = []
        for yi in classes:
    ...
    img_save_path= f"/root/autodl-tmp/MinimaxDiffusion/results/latent/tsne/Vis_tsne_{str(name)}.pdf"
